...
class TestRoleBasedAccess:
    ...
    def test_admin_full_access(self, client, admin_token, sample_shipment_data, reset_db):
        ...
        # Create
        create = client.post("/shipment", json=sample_shipment_data, headers={"Authorization": f"Bearer {admin_token}"})
        assert create.status_code == 201

        # Read
        read = client.get("/shipments", headers={"Authorization": f"Bearer {admin_token}"})
        ...
        # Delete
        delete = client.delete("/shipment/12701", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    def test_courier_limited_access(self, client, courier_token, sample_shipment_data, reset_db):
        ...
        # Can read
        read = client.get("/shipments", headers={"Authorization": f"Bearer {courier_token}"})
        assert read.status_code == 200
        ...
        # Cannot delete
        delete = client.delete("/shipment/12701", headers={"Authorization": f"Bearer {courier_token}"})
    ...
    def test_customer_restricted_access(self, client, customer_token, sample_shipment_data, reset_db):
        ...
        # Can read
        read = client.get("/shipments", headers={"Authorization": f"Bearer {customer_token}"})
        assert read.status_code == 200
        ...
        # Cannot delete
        delete = client.delete("/shipment/12701", headers={"Authorization": f"Bearer {customer_token}"})
        assert delete.status_code == 403
...
class TestSecurityVulnerabilities:
    ...
    def test_sql_injection_attempt(self, client, admin_token, reset_db):
        ...
        response = client.get(
            "/shipment/12701'; DROP TABLE shipments; --", headers={"Authorization": f"Bearer {admin_token}"}
        )
