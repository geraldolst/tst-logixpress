...
def courier_token(client, reset_db):
    ...
    response = client.post("/auth/login", json={"username": "courier", "password": "courier123"})
    return response.json()["access_token"]
...
def customer_token(client, reset_db):
    ...
    response = client.post("/auth/login", json={"username": "customer", "password": "customer123"})
    return response.json()["access_token"]
...
def sample_shipment_data():
    ...
    return {
        "package_details": {"content": "Test Package", "weight": 5.5, "dimensions": "30x20x15", "fragile": True},
        "recipient": {
            "name": "John Doe",
        ...
        "seller": {"name": "Test Store", "email": "sales@teststore.com", "phone": "+6281234567891"},
        "destination_code": 11002,
    }
