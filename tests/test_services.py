...
from app.api.schemas.shipment import PackageDetails, Recipient, Seller, ShipmentCreate
from app.core.exceptions import DuplicateEntity, EntityNotFound
...
class TestAuthService:
    ...
    def test_register_user_success(self, reset_db):
        ...
        register_data = RegisterRequest(
            username="newuser", email="newuser@example.com", password="password123", role="customer"
        )
    ...
    def test_register_user_duplicate_username(self, reset_db):
        ...
        register_data = RegisterRequest(username="admin", email="newemail@example.com", password="password123")
...
class TestShipmentService:
    ...
    def test_create_shipment(self, reset_db):
        ...
        shipment_data = ShipmentCreate(
            package_details=PackageDetails(content="Test Package", weight=5.5, fragile=True),
            recipient=Recipient(
                name="John Doe", email="john@example.com", phone="+6281234567890", address="Test Address"
            ),
            seller=Seller(name="Test Store", email="store@example.com", phone="+6281234567891"),
