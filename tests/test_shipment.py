...
class TestShipmentCRUD:
    ...
    def test_create_shipment_as_customer(self, client, customer_token, sample_shipment_data, reset_db):
        ...
        response = client.post(
            "/shipment", json=sample_shipment_data, headers={"Authorization": f"Bearer {customer_token}"}
        )
        assert response.status_code == 201
    ...
    def test_create_shipment_as_courier_forbidden(self, client, courier_token, sample_shipment_data, reset_db):
        ...
        response = client.post(
            "/shipment", json=sample_shipment_data, headers={"Authorization": f"Bearer {courier_token}"}
        )
    ...
    def test_get_shipment_by_id(self, client, admin_token, reset_db):
        ...
        response = client.get("/shipment/12701", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    def test_get_shipment_not_found(self, client, admin_token, reset_db):
        ...
        response = client.get("/shipment/99999", headers={"Authorization": f"Bearer {admin_token}"})
        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()

    def test_get_all_shipments(self, client, admin_token, reset_db):
        """Test getting all shipments"""
        response = client.get("/shipments", headers={"Authorization": f"Bearer {admin_token}"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_shipments_filter_by_status(self, client, admin_token, reset_db):
        ...
        response = client.get("/shipments?status=placed", headers={"Authorization": f"Bearer {admin_token}"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_shipments_filter_by_destination(self, client, admin_token, reset_db):
        ...
        response = client.get("/shipments?destination_code=11002", headers={"Authorization": f"Bearer {admin_token}"})
        assert response.status_code == 200
        data = response.json()
        for shipment in data:
    ...
    def test_get_shipments_with_limit(self, client, admin_token, reset_db):
        ...
        response = client.get("/shipments?limit=1", headers={"Authorization": f"Bearer {admin_token}"})
        assert response.status_code == 200
        data = response.json()
        assert len(data) <= 1
    ...
    def test_update_shipment_status(self, client, admin_token, reset_db):
        ...
        response = client.patch(
            "/shipment/12701", json={"current_status": "in_transit"}, headers={"Authorization": f"Bearer {admin_token}"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_update_shipment_invalid_transition(self, client, admin_token, reset_db):
        ...
        response = client.patch(
            "/shipment/12701", json={"current_status": "delivered"}, headers={"Authorization": f"Bearer {admin_token}"}
        )
    ...
    def test_update_shipment_package_details(self, client, admin_token, reset_db):
        ...
        response = client.patch(
            "/shipment/12701",
            json={"package_details": {"content": "Updated content", "weight": 10.0, "fragile": True}},
            headers={"Authorization": f"Bearer {admin_token}"},
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_delete_shipment_as_admin(self, client, admin_token, reset_db):
        ...
        response = client.delete("/shipment/12701", headers={"Authorization": f"Bearer {admin_token}"})
        assert response.status_code == 200
        assert "deleted" in response.json()["message"].lower()

        # Verify shipment is deleted
        get_response = client.get("/shipment/12701", headers={"Authorization": f"Bearer {admin_token}"})
    ...
    def test_delete_shipment_as_courier_forbidden(self, client, courier_token, reset_db):
        ...
        response = client.delete("/shipment/12701", headers={"Authorization": f"Bearer {courier_token}"})
        assert response.status_code == 403

    def test_delete_shipment_not_found(self, client, admin_token, reset_db):
        """Test deleting non-existent shipment"""
        response = client.delete("/shipment/99999", headers={"Authorization": f"Bearer {admin_token}"})
        assert response.status_code == 404
